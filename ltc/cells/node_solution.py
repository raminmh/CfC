...
class NODESolution(tf.keras.layers.Layer):
    ...
    def __init__(self,
        ...
        super(NODESolution, self).__init__(**kwargs)
    ...
    def call(self, inputs, states):
        ...
        x = self.B * self.f(h) * t + self.W_l * tf.math.log(tf.exp(h)+1) + self.A
        # x = - self.B * tf.exp(- t * 1/(self.tau + self.f(h))) + self.f(-h) + self.A
